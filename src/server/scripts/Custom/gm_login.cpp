	...
	void OnLogin(Player* player)
	{
		if (sWorld->getBoolConfig(CONFIG_GMLOGIN_ENABLED))
		{
			if (player->GetSession()->GetSecurity() >= SEC_MODERATOR)
			{
				...
				handler.PSendSysMessage("|cff00ff00Ва?уровен?учетно?записи:|r %u", gmlvl);
				handler.PSendSysMessage("|cff00ff00Ва?IP:|r %s", player_ip.c_str()); 
				handler.PSendSysMessage("|cff00ff00Сейчас|r %u |cff00ff00игроко?онлайн|r", playeronline);
				handler.PSendSysMessage("|cff00ff00Открытие тикеты:|r %u", tickets);
				handler.PSendSysMessage("|cff00ff00Время работы сервер?|r %s", uptime.c_str());
				handler.PSendSysMessage("|cff00ff00Удач??пр?тной игры|r");
				handler.PSendSysMessage("|cffff0000=================================|r");
				return;
			}
		}
...
public:
	...
	void OnLogin(Player* player)
	{
		...
		{
			char msg[250];
			snprintf(msg, 250, "|cffff0000[Аннонс входа]:|r Модерато?%s заше??игрово?ми?", player->GetName());
		...
		{
			char msg[250];
			snprintf(msg, 250, "|cffff0000[Аннонс входа]:|r Игрово?мастер %s заше??игрово?ми?", player->GetName());
			sWorld->SendServerMessage(SERVER_MSG_STRING, msg);
		}
		...
		{
			char msg[250];
			snprintf(msg, 250, "|cffff0000[Аннонс входа]:|r Администрато?%s заше??игрово?ми?", player->GetName());
			sWorld->SendServerMessage(SERVER_MSG_STRING, msg);
		}
	}
};
