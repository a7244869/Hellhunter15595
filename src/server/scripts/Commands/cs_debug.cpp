    ...
    static bool HandleDebugMoveflagsCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                target->m_movementInfo.WriteToPacket(data);
...
*/
    ...
    static bool HandleDebugUnRootCommand(ChatHandler* handler, char const* /*args*/)
    {
        Player* player = handler->GetSession()->GetPlayer();
    ...
    static bool HandleDebugCombatCommand(ChatHandler* handler, char const* /*args*/)
    {
        Player* player = handler->GetSession()->GetPlayer();
        handler->PSendSysMessage("Last spell that got you in combat was %u, casted by %s", player->m_lastCombatspell, player->m_lastCombatCaster.c_str());
        return true;
    }
};
