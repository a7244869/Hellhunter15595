...
enum Spells
{
	...
	SPELL_FLAME_ARROW_DAMAGE = 74944,
...
enum Events
{
    ...
    EVENT_ARCHER_SHOOT,
	//Fire Patch
	EVENT_SPELL_LAVA_PATCH,
	EVENT_SPELL_FLAME_ARROW_DAMAGE,
};
...
class mob_fire_patch : public CreatureScript
{
public:
	mob_fire_patch() : CreatureScript("mob_fire_patch") {}

	struct mob_fire_patchAI : public ScriptedAI
	{
		mob_fire_patchAI(Creature *c) : ScriptedAI(c) {}

		EventMap events;

		void Reset()
		{
			events.ScheduleEvent(EVENT_SPELL_LAVA_PATCH, 1000);
			DoZoneInCombat();
		}
		...
		void UpdateAI(const uint32 diff)
		{
			...
			{
			case EVENT_SPELL_LAVA_PATCH:
			{
				DoCast(SPELL_LAVA_PATCH);
				events.ScheduleEvent(EVENT_SPELL_LAVA_PATCH, 1000);
				break;
			}
			}
	...
	CreatureAI* GetAI(Creature* pCreature) const
	{
		return new mob_fire_patchAI(pCreature);
	}
...
public:
	...
	{
		mob_twilight_archerAI(Creature *c) : ScriptedAI(c) {}

		EventMap events;

		void Reset()
		{
			events.ScheduleEvent(EVENT_SPELL_FLAME_ARROW_DAMAGE, urand(1500,3500));
			DoZoneInCombat();
		}
		...
		void UpdateAI(const uint32 diff)
		{
			...
			switch (events.ExecuteEvent())
			{
			case EVENT_SPELL_FLAME_ARROW_DAMAGE:
			{
				if (Unit *pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 60, true))
					DoCast(pTarget, SPELL_FLAME_ARROW_DAMAGE);
				events.ScheduleEvent(EVENT_SPELL_FLAME_ARROW_DAMAGE, urand(1500, 3500));
				break;
			}
			}
	...
	CreatureAI* GetAI(Creature* pCreature) const
	{
		return new mob_twilight_archerAI(pCreature);
	}
