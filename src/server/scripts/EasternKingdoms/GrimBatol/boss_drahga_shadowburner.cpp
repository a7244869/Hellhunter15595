...
{
    // Drahgas Spells
    SPELL_BURNING_SHADOWBOLT    = 75245, //È¼ÉÕ°µÓ°¼ý
    SPELL_BURNING_SHADOWBOLT_H    = 90915,//È¼ÉÕ°µÓ°¼ýH

    SPELL_INVOCATION_OF_FLAME    = 75218, // Summons Trigger at Random Location ÕÙ»½»ðÔªËØ
    SPELL_INVOCATION_TRIGGER    = 75222, // Summons & Visual is casted by the Trigger

    SPELL_TWILIGHT_PROTECTION    = 76303,//Äº¹â±£»¤

    // Valionas Spells
    SPELL_VALIONAS_FLAME        = 75321, //ÍßÁ¦Å·ÄÈÖ®Ñæ
    SPELL_SHREDDING_SWIPE        = 75271, //ËéÁÑ»áÉ¨
    SPELL_SEEPING_TWILIGHT        = 75318, // wowhead says 75317 but this spell gives the visual aura//Äº¹âÉøÍ¸
	SPELL_SEEPING_TWILIGHT_DAMAGE  = 75317,//Äº¹âÉøÍ¸µÄÉËº¦
    SPELL_DEVOURING_FLAMES_H    = 90950, //ÍÌÊÉÁÒÑæ
    SPELL_TWILIGHT_SHIFT        = 75328, //Äº¹â±ä»»

    // Invoked Flame Spirits Spells
    SPELL_AURA_NOVA                = 75235, //ÕÙ»½µÄ»ðÑæ
    SPELL_CONCENTRATE_FIRE_AURA    = 82850, //È¼ÉÕÄýÊÓ

    SPELL_SUPERNOVA                = 75238, //»õÔ´ÕÙ»½
...
enum Events
{
	...
	EVENT_SEEPING_TWILIGHT_DAMAGE     = 8,
};
...
{
    {-400.613f, -671.578f, 265.896f, 0.102f},    // Drahga Point from who he jump down µÄÀ²¿¨ÌøÏÂµÄÎ»ÖÃ
    {-388.189f, -668.078f, 280.316f, 3.470f},    // Valionas Way to the Platform  ÍßÁ¦Å·ÄÈÂäµØÎ»ÖÃ
    {-435.54f, -695.072f, 280.316f, 3.4010f},
    {-435.54f, -695.072f, 268.687f, 3.4010f},    // Valiona first land Position ÍßÁ¦Å·ÄÈµØÒ»ÏÂÂäµØ
...
public:
    ...
    struct boss_drahga_shadowburnerAI : public ScriptedAI
    {
        ...
        void Reset()
        {
			...
			me->RemoveAura(VEHICLE_SPELL_RIDE_HARDCODED);
			me->RemoveAllAuras();
            if (pInstance)
		...
		{
			std::list<Creature*> cList;
			me->GetCreatureListWithEntryInGrid(cList, NPC_VALIONA, 200.0f);
			if (!cList.empty())
				for (std::list<Creature*>::const_iterator itr = cList.begin(); itr != cList.end(); ++itr)
					(*itr)->DespawnOrUnsummon();
		}
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (phase == PHASE_CASTER_PHASE && !HealthAbovePct(30))
            {
				...
				if (Creature *valiona = me->SummonCreature(NPC_VALIONA, -375.33f, -667.291f, 250.0f, 3.29545f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,8000))
...
class mob_seeping_tswilight : public CreatureScript
{
public:
	mob_seeping_tswilight() : CreatureScript("mob_seeping_tswilight") {}

	struct mob_seeping_tswilightAI : public ScriptedAI
	{
		mob_seeping_tswilightAI(Creature *c) : ScriptedAI(c) {}

		EventMap events;

		void Reset()
		{
			events.ScheduleEvent(EVENT_SEEPING_TWILIGHT_DAMAGE, 1000);
			DoZoneInCombat();
		}
		...
		void UpdateAI(const uint32 diff)
		{
			...
			{
			case EVENT_SEEPING_TWILIGHT_DAMAGE:
			{
					DoCast(SPELL_SEEPING_TWILIGHT_DAMAGE);
					events.ScheduleEvent(EVENT_SEEPING_TWILIGHT_DAMAGE, 1000);
				break;
			}
			}
	...
	{
		return new mob_seeping_tswilightAI(pCreature);
	}
