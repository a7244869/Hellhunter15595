...
# define _TRINITY_REALM_CONFIG  "hhauthserver.conf"
#endif
...
extern int main(int argc, char **argv)
{
    ...
    if (!ConfigMgr::Load(cfg_file))
    {
        ...
        printf("Verify that the file exists and has \'[hhauthserver]\' written in the top of the file!\n");
        return 1;
    }
	sLog->outInfo(LOG_FILTER_WORLDSERVER, _StringToUTF8("Nerock制作组"));
	...
	sLog->outInfo(LOG_FILTER_WORLDSERVER, " **    ** **   **         **   **      **       **    **              **    **   ");
	sLog->outInfo(LOG_FILTER_WORLDSERVER, " **     ****   ********   ***    **       ****           ********   *****     **            ");
	sLog->outInfo(LOG_FILTER_WORLDSERVER, _StringToUTF8("Version V1.0  Beta     "));
	sLog->outInfo(LOG_FILTER_WORLDSERVER, _StringToUTF8("NEROCK制作?      TrinityCore4.34 "));
	sLog->outInfo(LOG_FILTER_WORLDSERVER, _StringToUTF8("联系方式 mail & QQ: 513157129& game.techbang01@gmail.com"));
	sLog->outInfo(LOG_FILTER_WORLDSERVER, _StringToUTF8("本端为V1.0测试免费版本切勿花钱购买，免费版本限制人数5人.如果需要无限制商业版请联系NEROCK \n "));
	sLog->outInfo(LOG_FILTER_WORLDSERVER, "");
    ...
    {
        sLog->outError(LOG_FILTER_AUTHSERVER, "hhauthserver can not bind to %s:%d", bind_ip.c_str(), rmport);
        return 1;
    }
...
#ifdef _WIN32
    {
        ...
        if (Prio)
        {
            if (SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS))
                sLog->outInfo(LOG_FILTER_AUTHSERVER, "hhauthserver process priority class has been set to HIGH");
            else
