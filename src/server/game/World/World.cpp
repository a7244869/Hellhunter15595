...
void World::LoadConfigSettings(bool reload)
{
	...
	m_int_configs[CONFIG_MAIL_GM] = ConfigMgr::GetIntDefault("Hellhunter.mail.gm", 90);
	m_int_configs[CONFIG_QUESTS_9999] = ConfigMgr::GetIntDefault("Hellhunter.quests", 25);
	...
	m_bool_configs[CONFIG_FAKE_RANDOM_ONLINE_OFFLINE]		  = ConfigMgr::GetBoolDefault("Fake.ONLINE.System", false);
	m_bool_configs[CONFIG_FAKE_LEVEL_UP]					  = ConfigMgr::GetBoolDefault("Fake.LEVELUP.System", false);
	...
	m_int_configs[CONFIG_FAN_ACHIVEMENT_SPELL_VALUTA] = ConfigMgr::GetIntDefault("TER-Server_FAN_ACHIVEMENT_SPELL_VALUTA", 0);   //FAN SPELL CLIENT+, OFF CRASH
	...
	m_float_configs[CONFIG_SPEED_GAME] = ConfigMgr::GetFloatDefault("Hellhunter.SpeedGame", 1.0f);

	m_bool_configs[CONFIG_NO_CAST_TIME] = ConfigMgr::GetBoolDefault("Hellhunter.NoCastTime", false);

	m_bool_configs[CONFIG_NO_COOLDOWN] = ConfigMgr::GetBoolDefault("Hellhunter.NoCooldown", false);

	m_bool_configs[CONFIG_HURT_IN_REAL_TIME] = ConfigMgr::GetBoolDefault("Hellhunter.HurtInRealTime", false);

	

		m_float_configs[CONFIG_ATTACKSPEED_PLAYER] = ConfigMgr::GetFloatDefault("Hellhunter.AttackSpeedForPlayer", 1.0f);

	m_float_configs[CONFIG_ATTACKSPEED_ALL] = ConfigMgr::GetFloatDefault("Hellhunter.AttackSpeedForMobs", 1.0f);

	m_bool_configs[CONFIG_FAST_FISHING] = ConfigMgr::GetBoolDefault("Hellhunter.Fishing.time-off", false);

	m_bool_configs[CONFIG_NO_GREY_AGGRO] = ConfigMgr::GetBoolDefault("Hellhunter.No_Grey_Aggro", false);

	m_float_configs[CONFIG_RESPAWNSPEED] = ConfigMgr::GetFloatDefault("Hellhunter.Respawn-Speed", 1.0f);

	m_bool_configs[CONFIG_GAIN_HONOR_GUARD] = ConfigMgr::GetBoolDefault("Hellhunter.Honor-Kill-Guard", false);

	m_bool_configs[CONFIG_GAIN_HONOR_ELITE] = ConfigMgr::GetBoolDefault("Hellhunter.Honor-Kill-Elite", false);
	...
	m_bool_configs[CONFIG_FAST_EN_TER884] = ConfigMgr::GetBoolDefault("Hellhunter.fast-enchant", false);
	...
	m_int_configs[CONFIG_DEAD_FAST_REST_TER884_P1] = ConfigMgr::GetIntDefault("dead_1", 10);
	m_int_configs[CONFIG_DEAD_FAST_REST_TER884_P2] = ConfigMgr::GetIntDefault("dead_2", 20);
	m_int_configs[CONFIG_DEAD_FAST_REST_TER884_P3] = ConfigMgr::GetIntDefault("dead_3", 30);
	m_int_configs[CONFIG_DEAD_FAST_REST_TER884_P4] = ConfigMgr::GetIntDefault("dead_4", 45);
	m_int_configs[CONFIG_DEAD_FAST_REST_TER884_P5] = ConfigMgr::GetIntDefault("dead_5", 60);
	m_int_configs[CONFIG_DEAD_FAST_REST_TER884_P6] = ConfigMgr::GetIntDefault("dead_6", 90);
	...
	m_float_configs[VAS_Config_xPlayer] = ConfigMgr::GetIntDefault("VAS.AutoBalance.XPlayer", 1.0f);
	m_float_configs[VAS_Min_D_Mod] = ConfigMgr::GetIntDefault("Min.D.Mod", 0.10f);
	m_float_configs[VAS_Min_HP_Mod] = ConfigMgr::GetIntDefault("Min.HP.Mod", 0.20f);
