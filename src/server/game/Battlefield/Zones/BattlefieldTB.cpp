...
bool BattlefieldTB::SetupBattlefield()
{
    ...
    for(uint8 i = 0; i<TB_MAX_WORKSHOP; i++)
    {
        ...
        {
            ws->GiveControlTo(GetAttackerTeam(), true); // Update control of this point
        ...
        else
        {
            m_Data32[BATTLEFIELD_TB_DATA_CAPTURED_FORT_ATT]--;
            m_Data32[BATTLEFIELD_TB_DATA_CAPTURED_FORT_DEF]++;
            ws->GiveControlTo(GetDefenderTeam(), true); // Update control of this point (Keep TBWorkShop= to deffender team)
        }
