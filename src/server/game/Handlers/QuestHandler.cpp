        ...
        switch (object->GetTypeId())
        {
            ...
            case TYPEID_PLAYER:
            {
                ...
                if (!creatureQGiver || !(sScriptMgr->OnQuestReward(_player, creatureQGiver, quest, reward)))
                {
                    ...
                    {
                        if ((nextQuest->IsAutoAccept() || nextQuest->HasFlag(QUEST_FLAGS_AUTO_SUBMIT) || nextQuest->HasFlag(QUEST_FLAGS_AUTO_TAKE))
                            && _player->CanAddQuest(nextQuest, true) && _player->CanTakeQuest(nextQuest, true, true))
                        {
