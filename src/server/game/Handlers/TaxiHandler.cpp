...
void WorldSession::HandleMoveSplineDoneOpcode(WorldPacket& recvData)
{
    ...
    uint32 splineID = recvData.read<uint32>();

    if (_player->GetSplineId() != splineID)
        return;

    Movement::MoveSplineInit init(_player);
    init.ForceDone();

    MovementInfo movementInfo;                              // used only for proper packet read
    movementInfo.ReadFromPacket(recvData);

    HandleMovementInfo(movementInfo, recvData.GetOpcode(), recvData.size()-4, _player->m_mover);
